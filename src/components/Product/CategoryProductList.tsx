...
export function CategoryProductsList() {
	...
	>(
		(index) =>
			`/api/products/all/category?category_name=${category}&page=${
				index + 1
			}&limit=${PAGE_SIZE}`,
		fetcher
	)
	...
	const categorizedProducts = flattenedData
		? flattenedData[0]?.category_group.map((item: any) => item.product)
		: []

	const products: ProductType[] = categorizedProducts
		? // @ts-ignore - TS doesn't know that `flat()` returns an array
		  [].concat(...categorizedProducts)
		: ([] as ProductType[])
			...
			<div>
				...
				<main className=" mx-auto px-4 sm:px-6 lg:px-8">
					<div className="w-full bg-white z-10 flex items-center justify-between sticky top-0 pt-4 pb-6 border-b border-gray-200">
						<Heading size="h3">{CATEGORIES[category] ?? 'Explore'}.</Heading>
